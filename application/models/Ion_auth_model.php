...
class Ion_auth_model extends CI_Model {
    ...
    public function register($identity, $password, $email, $additional_data = array(), $groups = array()) {
        ...
        if (isset($additional_data['active'])) {
    ...
    public function login($identity, $password, $remember = FALSE) {
        ...
        $query = $this->db->select($this->identity_column . ', email, id, password, active, last_login, facebook_id')
                ->where($this->identity_column, $identity)
                ->limit(1)
                ->order_by('id', 'desc')
                ->get($this->tables['users']);
        ...
        if ($query->num_rows() === 1) {
            ...
            // Facebook login
            if (substr($password, 0, 2) === 'FB') {
                $fbId = substr($password, 2);
                $password = TRUE;
                if (!$this->facebook_check($user->email, $fbId)) {
                    $password = FALSE;
                }
            // Login Google
            } elseif (substr($password, 0, 2) === 'GG') {
                $fbId = substr($password, 2);
                $password = TRUE;
                if (!$this->google_check($user->email, $fbId)) {
                    $password = FALSE;
                }
            } else {
                ...
                switch ($type) {
                    case '$S$':
                        ...
                        $password = user_check_password_new($password, $user->password);
                        break;
                    default:
...
//                            }
    ...
    public function facebook_check($email, $id) {
        return $this->db
                        ->where("email", $email)
    ...
    public function google_check($email, $id) {
        return $this->db
                        ->where("email", $email)
                        ->where("google_id", $id)
                        ->count_all_results($this->tables['users']) > 0;
    ...
    public function update_user($email, array $data) {
        $this->db->set($data);
        $this->db->where($this->identity_column, $email);
